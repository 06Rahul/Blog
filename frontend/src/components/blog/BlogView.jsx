...
export const BlogView = () => {
  ...
  const isOwner = user && (blog.author?.id === user.id || blog.authorId === user.id);
      ...
      <article className="bg-white rounded-lg shadow-md p-8">
        ...
        <header className="mb-8">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
            ...
            <Link 
              to={`/profile/${blog.author?.username || blog.authorUsername}`}
              className="font-bold text-primary-600 hover:text-primary-700 hover:underline transition-all"
            >
              {blog.author?.firstName && blog.author?.lastName 
                ? `${blog.author.firstName} ${blog.author.lastName}` 
                : (blog.author?.username || blog.authorUsername || 'Unknown')}
            </Link>
            {blog.publishedAt && (
