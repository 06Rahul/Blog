...
export const ChatList = () => {
  ...
  const handleStartConversation = async (userId) => {
    try {
      ...
      navigate(conversation?.id ? `/messages/${conversation.id}` : '/messages');
    } catch (err) {
            ...
            {conversations.map((conv) => (
              ...
              >
                <div className="relative">
                  ...
                  <span className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-white dark:border-gray-800 ${conv.otherUserOnline ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-500'}`}></span>
                  {conv.unreadCount > 0 && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={() => setShowNewConversationModal(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md m-4 max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
              ...
              <button onClick={() => setShowNewConversationModal(false)} className="text-gray-500 hover:text-gray-700">x</button>
            </div>
            <div className="flex-1 overflow-y-auto p-2">
              {loadingUsers ? (
                ...
                <>
                  ...
                  {[...followers, ...following].map((followUser) => (
                    <button key={followUser.id} onClick={() => handleStartConversation(followUser.id)} className="w-full p-3 flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg text-left">
                      <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center font-bold text-blue-600">{followUser.username[0]}</div>
                      <span className="font-medium text-gray-900 dark:text-white">{followUser.username}</span>
                    </button>
                  ))}
                </>
              )}
