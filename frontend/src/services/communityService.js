...
export const communityService = {
  ...
  discoverCommunities: async (page = 0, size = 10, filter = 'featured', search = '', category = '') => {
    const params = new URLSearchParams({ page, size, filter });
    if (search) params.append('search', search);
    if (category) params.append('category', category);
    const response = await api.get(`/communities/discover?${params.toString()}`);
    return response.data;
  },

  getMyJoinedCommunities: async (page = 0, size = 50) => {
    const response = await api.get(`/communities/mine/joined?page=${page}&size=${size}`);
    return response.data;
  },

  getMyOwnedCommunities: async (page = 0, size = 50) => {
    const response = await api.get(`/communities/mine/owned?page=${page}&size=${size}`);
    return response.data;
  },
